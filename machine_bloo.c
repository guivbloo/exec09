 ...
 void bloo_init (const char *boot_rom_file)
{
	...
	device_define (ram , 0, BLOO_RAM_BASE, BLOO_RAM_SIZE, MAP_READWRITE );

	/* $C000 and $C001 for 6850 */
	uart = mc6850_create();
    device_define (uart, 0, 0xC000, BUS_MAP_SIZE, MAP_READWRITE);
	
	/* 12K ROM from D000 to FFFF */
    rom = rom_create (boot_rom_file, BLOO_ROM_SIZE);
    device_define (rom , 0, BLOO_ROM_BASE, BLOO_ROM_SIZE, MAP_READWRITE);
}


 
 struct machine bloo_machine =
{
	.name = "bloo",
	.fault = fault,
	.init = bloo_init,
