...
struct hw_device *ym2149_create (unsigned long size)
{
	struct ym2149 *psg = malloc(sizeof(struct ym2149));
	struct hw_device *dev;
	memset(psg, 0x00, sizeof(struct ym2149));
	dev =  device_create (&ym2149_class, size, psg);
	ym2149_reset(dev);
	return dev;
}